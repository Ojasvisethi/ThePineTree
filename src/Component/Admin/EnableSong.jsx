...
function EnableSong({ setSelect }) {
  ...
  return (
    <div className={styles.delete}>
      ...
      ></FaArrowLeft>
      <h1 style={{ fontSize: "3rem" }}>Enable Song</h1>
      <div
        style={{
          display: "flex",
          alignItems: "center",
...
async function enableSongs(songIds) {
  try {
    const response = await fetch("http://localhost:3000/music/enable", {
      method: "POST",
      headers: {
...
function SongList({ search, navigate }) {
  ...
  const disableSongsMutation = useMutation(enableSongs, {
    onSuccess: () => {
      toast.success("Songs Enabled");
      navigate("/admin");
    },
    onMutate: (variables) => {
